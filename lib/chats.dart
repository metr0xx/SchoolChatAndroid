...
void fillChats2(var incomming){
    ...
    for(int i = 0; i < chatDatas.length; i++) {
      if(chatDatas[i].id == chatinfo['id'] || chatDatas[i].name == chatinfo['name']) {
        noSame = false;
        break;
      }
    }
...
class Chats extends StatelessWidget {
  @override
  Text smallclear = Text('', style: TextStyle(fontSize: 6),);
  Text midclear = Text('', style: TextStyle(fontSize: 11));
  Text bigclear = Text('', style: TextStyle(fontSize: 40),);
  String name = '';
 ...
 bool checker(name) {
  ...
  TextButton plus = TextButton(onPressed: () {
    showDialog(
      context: context,
      builder: (context) {
        return Dialog(
               ...
               Stack(
                children: <Widget>[
                  Container(
                    ...
                    // ignore: todo
                    child:Text( //TODO Center(child: Text(..............))
